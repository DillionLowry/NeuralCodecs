...
public partial class DSP
{
    ...
    public AudioSignal HighPass(
        ...
        int zeros = 51)
    {
        cutoffs = cutoffs.to_type(float32) / signal.SampleRate;
        var filtered = zeros_like(signal.AudioData);
        ...
        for (int i = 0; i < cutoffs.size(0); i++)
        {
            ...
            var n = arange(-zeros / 2, (zeros / 2) + 1);
            var h = 2 * cutoff.item<float>() * sinc(2 * cutoff * n);
            h = -h;  // Invert for high-pass
            h[zeros / 2] += 1;  // Add impulse

            // Apply Hamming window
            var window = 0.54f - (0.46f * cos(2 * (float)Math.PI * (n + (zeros / 2)) / zeros));
            h *= window;
            ...
            // Apply filtering
            filtered[i] = nn.functional.conv1d(
        ...
        float val = 0.0f)
    {
        ...
        var binsHz = linspace(0, signal.SampleRate / 2, nbins, device: signal.Device);
        ...
        signal.StftData = mag * exp(Complex.ImaginaryOne * phase);
        ...
        float val = 0.0f)
    {
        ...
        if (any(tminS >= tmaxS).item<bool>()) // todo : check this
        {
        ...
        var binsT = linspace(0, signal.SignalDuration, nt, device: signal.Device);
        ...
        float hopDuration)
    {
        ...
        var norm = ones_like(unfolded);
        var normFolded = nn.functional.fold(
            norm,
            output_size: _paddedSignalLength,
            kernel_size: windowLength,
            stride: hopLength);
    ...
    public AudioSignal Preemphasis(AudioSignal signal, float coef = 0.85f)
    {
        ...
        var kernel = tensor(new float[] { 1, -coef, 0 })
            .reshape(1, 1, -1)
            .to(signal.Device);

        var x = signal.AudioData.reshape(-1, 1, signal.SignalLength);
        x = nn.functional.conv1d(x, kernel, padding: 1);
